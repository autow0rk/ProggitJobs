"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const deleteOldJobs_1 = __importDefault(require("./deleteOldJobs"));
function Init(prismaClient) {
    return __awaiter(this, void 0, void 0, function* () {
        yield (0, deleteOldJobs_1.default)(prismaClient);
        yield getSubredditFilters(prismaClient);
    });
}
function getSubredditFilters(prismaClient) {
    return __awaiter(this, void 0, void 0, function* () {
        // async function getSubredditFilterOptions(){
        // const subredditFilterOptions = await prismaClient.$queryRaw`SELECT DISTINCT fromSubreddit FROM Job`;
        const fromSubredditFiltersFromDB = yield prismaClient.job.findMany({
            distinct: ['fromSubreddit'],
            select: {
                fromSubreddit: true
            }
        });
        const filtersAsArray = fromSubredditFiltersFromDB.map((subredditFilter) => {
            return subredditFilter.fromSubreddit;
        });
        return filtersAsArray;
        // }
    });
}
exports.default = getSubredditFilters;
